...
async function main() {
  ...
  const customer3 = await prisma.customer.upsert({
    ...
    create: {
      ...
      mobile: '9876543212',
      businessName: 'Amit & Co.',
      customerType: 'DISTRIBUTOR',
      address: '789, Warehouse Zone, Bengaluru',
      status: 'LEAD',
  ...
  const product2 = await prisma.product.upsert({
    ...
    create: {
      ...
      currentStock: 200,
      minStockAlert: 30,
      warehouseLocation: 'Aisle B, Rack 3',
    },
  });
  ...
  const product3 = await prisma.product.upsert({
    ...
    create: {
      ...
      currentStock: 1000,
      minStockAlert: 100,
      warehouseLocation: 'Aisle C, Rack 2',
    },
  });
  ...
  const product4 = await prisma.product.upsert({
    ...
    create: {
      ...
      currentStock: 15,
      minStockAlert: 20,
      warehouseLocation: 'Aisle D, Rack 1',
    },
  });
  ...
  console.log(`Users: ${[admin, sales, warehouse, accounts].map(u => u.email).join(', ')}`);
  console.log(`Customers: ${[customer1, customer2, customer3, customer4].map(c => c.name).join(', ')}`);
  console.log(`Products: ${[product1, product2, product3, product4].map(p => p.name).join(', ')}`);
}
